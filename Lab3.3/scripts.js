...
function daysCount(date = String, daysNum) {
    ...
    var i = now.getDay();
    while (true) {
        if (days[i] == days[daysNum]) {
            document.getElementById('daysCount').innerHTML = "Число першого '" + days[daysNum] + "', місяця '" + mountNames[now.getMonth()] + "' = " + iteration;
            break;
        }
        i++;
        iteration++;
...
function calendarWithDateObj(date = new Date) {
    ...
    function getClassOfSelectedDay(tempDaysInMonth, futereM) {
        if (i == today && todayM == now.getMonth() && todayY == now.getFullYear()) {
            return 'today ';
        }
        else if (i - tempDaysInMonth == today && todayM == futereM && todayY == now.getFullYear()) {
            return 'today ';
        }
