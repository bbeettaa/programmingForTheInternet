...
class Quiz {
        ...
        if (this.proggres < 1) {
            this.interval = setInterval(() => {
                if (width >= this.expiredTime) {
                    ...
                    width++;
                    bar.style.width = width * (100 / this.expiredTime) + "%";
                    elem.innerHTML = width + " / " + this.expiredTime;
                }
    ...
    endQuiz() {
        alert(this.getName() + " з групи " + this.getGroup() + " здав тест на " + this.getCorrectAnswer() / this.getAllQuestions() * this.getAllQuestions());
